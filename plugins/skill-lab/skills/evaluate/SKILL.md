...
When validate fails and subjective eval is skipped, build criteria from findings so `--aggregate` is not called with an empty array. Map validator `severity: error` → criterion `severity: hard`:
